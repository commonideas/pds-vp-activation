...
function exchangeIdCandidates(exchangeIdInput) {
  const primary = normalizeExchangeId(exchangeIdInput);
  if (!primary) return [];

  const candidates = [primary];
  const withoutSuffix = primary.split('.')[0];
  if (withoutSuffix && withoutSuffix !== primary) {
...
async function lookupProfileByKxFilter(apiKey, exchangeId) {
  ...
  const body = await readKlaviyoBody(res);
  if (!res.ok) {
    console.warn('[klaviyo] profile filter by _kx failed', { status: res.status, exchangeId, body });
    return null;
  }
...
async function lookupProfileViaV2Exchange(apiKey, exchangeId) {
  const res = await fetch('https://a.klaviyo.com/api/v2/people/exchange', {
    method: 'POST',
    headers: {
      Authorization: `Klaviyo-API-Key ${apiKey}`,
      'Content-Type': 'application/x-www-form-urlencoded',
    },
    body: new URLSearchParams({ exchange_id: exchangeId }).toString(),
  });

  const body = await readKlaviyoBody(res);
  if (!res.ok) {
    console.warn('[klaviyo] v2 exchange failed', { status: res.status, exchangeId, body });
    return null;
  }

  const profileId = body?.id || body?.data?.id || body?.profile_id || null;
...
export async function getProfileByExchangeId(exchangeIdInput) {
  ...
  const candidates = exchangeIdCandidates(exchangeIdInput);
  if (!candidates.length) return null;

  for (const exchangeId of candidates) {
    const fromFilter = await lookupProfileByKxFilter(apiKey, exchangeId);
    if (fromFilter?.email) return fromFilter;

    const fromExchange = await lookupProfileViaV2Exchange(apiKey, exchangeId);
    if (fromExchange?.email) return fromExchange;
  }
