...
function isValidKlaviyoKx(value) {
  const companyId = (process.env.KLAVIYO_COMPANY_ID || 'RZDYF6').trim();
  if (!value || typeof value !== 'string' || !companyId) return false;

  const kx = value.trim();
  const parts = kx.split('.');
  if (parts.length !== 2 || parts[0].length < 8) return false;

  return parts[1].toUpperCase() === companyId.toUpperCase();
}
...
export async function resolveCampaignAccess({
  ...
  if (!email) {
    if (fromCampaignContext && isValidKlaviyoKx(kx)) {
      return grantCampaignBrowseAccess('campaign_kx');
    }
